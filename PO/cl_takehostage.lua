-----------------------------------------------------------------
--TakeHostage Fixed and Translated by Alex|God|Of|Glory--
------------------------------------------------------------------
...
function takeHostage()
	...
	if not canTakeHostage then 
		Notify("Hai bisogno di un arma e delle munizioni per prendere in ostaggio!")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			Notify("Nessuno vicino da prendere in ostaggio!")
		end 
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DisableControlAction(0,24,true) -- disable atck
			DisableControlAction(0,25,true) -- disable aim
			DisableControlAction(0,47,true) -- disable wpn
			DisableControlAction(0,58,true) -- disable wpn
			DisablePlayerFiring(GetPlayerPed(-1),true)
			local playerCoords = GetEntityCoords(GetPlayerPed(-1))
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Premi [G] per rilasciarlo, [H] per uccidere")
			if IsDisabledControlJustPressed(0,47) then --release	
				...
				Wait(100)
				releaseHostage()
			elseif IsDisabledControlJustPressed(0,74) then --k			
				holdingHostage = false
				holdingHostageInProgress = false 		
				local closestPlayer = GetClosestPlayer(2)
				target = GetPlayerServerId(closestPlayer)
